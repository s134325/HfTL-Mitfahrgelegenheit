...
public class VehicleResource
{
	...
	@ApiResponses(
			value = {
					@ApiResponse(code = 400,
							message = ValidationException.INVALID_UUID),
	...
	@Path("/user/{userUUID}")
	@ApiOperation(
			value = "Find vehicles by UUID of an user",
			notes = "Returns all vehicles by user UUID if the user is allowed to retrieve the information",
			response = VehicleResponse.class)
	@ApiResponses(value = {
	...
	@ApiOperation(value = "Create a vehicle",
			notes = "Creates a new vehicle based on the JSON",
	...
	@PUT
	@Path("/{vehicleUUID}")
	@ApiOperation(value = "Update a vehicle", notes = "Updates a vehicle",
			response = BaseResponse.class)
	@ApiResponses(value = {
			@ApiResponse(code = 400, message = "Invalid ID supplied"),
			@ApiResponse(code = 404, message = "Trip not found") })
	...
	@ApiOperation(value = "Deletes a vehicle",
			notes = "Deletes a vehicle by UUID", response = BaseResponse.class)
	@ApiResponses(value = {
			@ApiResponse(code = 400, message = "Invalid ID supplied"),
			@ApiResponse(code = 404, message = "Trip not found") })
	@Consumes({ MediaType.WILDCARD })
